...
function getProfileCreatedAtTimestamp(profile: ProfileMeta) {
  const timestamp = new Date(profile.createdAt).getTime();
  return Number.isNaN(timestamp) ? null : timestamp;
}
...
function getProfileSortValue(profile: ProfileMeta, key: ProfileSortKey) {
  switch (key) {
    case 'createdAt':
      return getProfileCreatedAtTimestamp(profile);
    case 'usage5h':
...
export const useAppStore = defineStore('app', {
  ...
  actions: {
    ...
    async refreshAppData(showSuccess = true) {
    ...
    async repairNetwork() {
      if (this.networkRepairing || this.acting) {
        return;
      }

      this.networkRepairing = true;
      try {
        const result = await backend.repairNetwork();
        this.notify(
          result.message || translate('notifications.networkRepairCompleted'),
          result.success ? 'success' : 'warning',
        );
      } catch (error) {
        this.notify(this.formatError(error), 'error');
      } finally {
        this.networkRepairing = false;
      }
    },
    ...
    openSettingsDialog() {
      this.settingsDialog = {
        ...
        restartCodexAfterSwitch: this.settings.restartCodexAfterSwitch,
        archivedSessionRetentionDays: this.settings.archivedSessionRetentionDays,
      };
    },
    ...
    async saveSettings(payload: {
      ...
      await this.runAction(async () => {
        this.appState = await backend.updateSettings({
          codexHomePath: payload.codexHomePath,
          minimizeToTrayOnClose: payload.minimizeToTrayOnClose,
          restartCodexAfterSwitch: payload.restartCodexAfterSwitch,
          archivedSessionRetentionDays: payload.archivedSessionRetentionDays,
        });
      ...
      try {
        await this.runAction(async () => {
          if (action === 'switch') {
            ...
            if (this.appState.settings.restartCodexAfterSwitch) {
              try {
                await backend.restartCodex();
                this.notify(translate('notifications.switchedAndRestarted'));
              } catch (error) {
                this.notify(
                  translate('notifications.switchedRestartFailed', {
