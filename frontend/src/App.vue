<template>
  <v-app>
        ...
        <div class="app-bar-inner">
          ...
          <div class="toolbar-actions">
            <v-btn
              ...
              :loading="loading"
              @click="store.refreshAppData(true)"
            >
              {{ t('toolbar.refresh') }}
            </v-btn>
            <v-btn
            ...
            <v-btn
              class="toolbar-btn"
              variant="outlined"
              :loading="networkRepairing"
              :disabled="acting || networkRepairing"
              @click="store.repairNetwork()"
            >
              {{ t('toolbar.networkRepair') }}
            </v-btn>
            <v-btn
              class="toolbar-btn"
              variant="outlined"
            ...
            <v-btn class="toolbar-btn" color="primary" :disabled="acting" @click="store.openCreateApiDialog()">
      ...
      <main class="main-shell">
        <div class="page-shell">
          ...
          <section class="table-panel">
            ...
            <div v-else class="profiles-table" :class="{ 'has-api-profiles': hasApiProfiles }">
                ...
                <table>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th class="plan-column">{{ hasApiProfiles ? t('profiles.headers.planOrUrl') : t('profiles.headers.plan') }}</th>
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="usage-column">
                        <div
                          v-if="profile.type === 'official' && profile.rateLimits.primary"
                          class="usage-cell"
                          :title="renderUsageResetTitle(profile.rateLimits.primary)"
                        >
                          <span class="usage-percent">{{ renderUsage(profile.rateLimits.primary, profile.type) }}</span>
                          <span v-if="renderUsageReset(profile.rateLimits.primary, 'time')" class="usage-reset">
                            {{ renderUsageReset(profile.rateLimits.primary, 'time') }}
                          </span>
                        </div>
                        <span v-else>{{ renderUsage(profile.rateLimits.primary, profile.type) }}</span>
                      </td>
                      <td class="usage-column">
                        <div
                          v-if="profile.type === 'official' && profile.rateLimits.secondary"
                          class="usage-cell"
                          :title="renderUsageResetTitle(profile.rateLimits.secondary)"
                        >
                          <span class="usage-percent">{{ renderUsage(profile.rateLimits.secondary, profile.type) }}</span>
                          <span v-if="renderUsageReset(profile.rateLimits.secondary, 'date')" class="usage-reset">
                            {{ renderUsageReset(profile.rateLimits.secondary, 'date') }}
                          </span>
                        </div>
                        <span v-else>{{ renderUsage(profile.rateLimits.secondary, profile.type) }}</span>
                      </td>
                      ...
                      <td class="actions-column">
                        <div class="row-actions">
                          <v-btn
                            ...
                            :loading="switchingProfileId === profile.id"
                            :disabled="acting || profile.disabled || profile.isActive"
    ...
    <SettingsDialog
      ...
      :minimize-to-tray-on-close="store.settingsDialog.minimizeToTrayOnClose"
      :restart-codex-after-switch="store.settingsDialog.restartCodexAfterSwitch"
      :archived-session-retention-days="store.settingsDialog.archivedSessionRetentionDays"
      :loading="acting"
...
const hasApiProfiles = computed(() => profiles.value.some((profile) => profile.type === 'api'));
...
type UsageResetDisplay = 'date' | 'time';

function renderUsageReset(window: RateLimitWindow | undefined, display: UsageResetDisplay) {
  const date = rateLimitResetDate(window);
  if (!date) {
    return '';
  }

  return display === 'date' ? formatUsageResetDate(date) : formatUsageResetTime(date);
}

function renderUsageResetTitle(window: RateLimitWindow | undefined) {
  const date = rateLimitResetDate(window);
  return date ? formatDateTime(date.toISOString()) : '';
}

function rateLimitResetDate(window: RateLimitWindow | undefined) {
  if (!window || typeof window.resetsAt !== 'number' || !Number.isFinite(window.resetsAt) || window.resetsAt <= 0) {
    return null;
  }
...
function formatUsageResetTime(date: Date) {
