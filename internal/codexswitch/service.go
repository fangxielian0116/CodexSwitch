...
type targetConfigWriteMode int

const (
	targetConfigWritePreserve targetConfigWriteMode = iota
	targetConfigWriteManaged
)
...
func NewService(options ServiceOptions) (*Service, error) {
	...
	if restartCodex == nil {
		restartCodex = restartCodexProcess
	}
...
func (s *Service) Close() error {
	...
	db := s.latencyHistoryDB
	s.latencyHistoryDB = nil
	return db.Close()
}
...
func (s *Service) UpdateSettings(input UpdateSettingsInput) (AppState, error) {
	...
	if input.MinimizeToTrayOnClose != nil {
		settings.MinimizeToTrayOnClose = *input.MinimizeToTrayOnClose
	}
	if input.RestartCodexAfterSwitch != nil {
		settings.RestartCodexAfterSwitch = *input.RestartCodexAfterSwitch
	}
	if input.ArchivedSessionRetentionDays != nil {
		if *input.ArchivedSessionRetentionDays <= 0 {
			return AppState{}, fmt.Errorf("归档对话过期时间必须大于 0 天")
		}
		settings.ArchivedSessionRetentionDays = *input.ArchivedSessionRetentionDays
	}
...
func (s *Service) UpdateAPIProfile(id string, input APIProfileInput) (AppState, error) {
	...
	if existing.Meta.IsActive {
		...
		if err := s.writeTargetProfile(settings.CodexHomePath, snapshot, targetConfigWriteManaged); err != nil {
			return AppState{}, err
		}
	}
...
func (s *Service) SwitchProfile(id string) (AppState, error) {
	...
	configMode := targetConfigWritePreserve
	if target.Meta.Type == ProfileTypeAPI || (target.Meta.Type == ProfileTypeOfficial && current.Type == ProfileTypeAPI) {
		configMode = targetConfigWriteManaged
	}

	if err := s.writeTargetProfile(settings.CodexHomePath, s.buildSnapshotFromExistingProfile(target), configMode); err != nil {
		return AppState{}, err
	}
...
func (s *Service) syncAndBuildState(autoSyncCurrent bool) (AppState, error) {
	...
	s.cleanupExpiredArchivedSessions(settings)
	current, currentSnapshot := s.scanCurrentProfile(settings.CodexHomePath)
	if autoSyncCurrent && currentSnapshot != nil && currentSnapshot.Meta.IsValid {
		if existing, err := s.loadProfile(currentSnapshot.Meta.ID); err != nil {
			...
			currentSnapshot.Meta.PlanType = existing.Meta.PlanType
...
func (s *Service) writeTargetProfile(codexHomePath string, snapshot *profileSnapshot, configMode targetConfigWriteMode) error {
	if snapshot == nil {
	...
	if configMode == targetConfigWritePreserve {
		return nil
	}

	configPath := filepath.Join(codexHomePath, "config.toml")
	currentRaw, err := readTextFile(configPath)
	if err != nil {
		if !isNotFound(err) {
			return fmt.Errorf("读取目标 config.toml 失败: %w", err)
		}
		currentRaw = ""
	}
	...
	if strings.TrimSpace(currentRaw) != "" {
		configRaw = mergeManagedConfigRaw(currentRaw, configRaw)
	} else {
		configRaw = normalizeConfigRaw(configRaw)
	}
	if err := safeWriteText(configPath, configRaw); err != nil {
