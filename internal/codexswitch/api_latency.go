...
func buildOfficialLatencyAvailabilityRequest(profile storedProfile) (*http.Request, error) {
	...
	req.Header.Set("Accept", "application/json")
	req.Header.Set("User-Agent", "CodexSwitch/0.1")
	if accountID := trimmedFirst(ctx.AccountID, profile.Meta.ChatGPTAccountID); accountID != "" {
...
// officialLatencyModelsURL returns the URL used to probe official account
// reachability. Official accounts authenticate with a ChatGPT OAuth access
// token, which is only accepted by the ChatGPT backend (e.g. the wham/usage
// endpoint used by the rate limit fetcher). Pointing the probe at the public
// OpenAI API (https://api.openai.com/v1/models) caused every test to fail with
// HTTP 403, so we delegate to the same URL builder used for rate limits.
func officialLatencyModelsURL(baseURL string) (string, error) {
	...
	return officialUsageURL(baseURL), nil
}
