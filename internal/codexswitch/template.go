...
func preserveStoredFields(next *ProfileMeta, existing *ProfileMeta, now time.Time) {
	...
	if shouldPreservePlanType(next, existing) {
...
func shouldPreservePlanType(next *ProfileMeta, existing *ProfileMeta) bool {
	if next == nil || existing == nil {
		return false
	}
	if next.Type != ProfileTypeOfficial || existing.Type != ProfileTypeOfficial {
		return false
	}
	if strings.TrimSpace(existing.PlanType) == "" {
		return false
	}
	return next.RateLimits.Status == RateLimitStatusIdle && next.RateLimits.Primary == nil && next.RateLimits.Secondary == nil
}
