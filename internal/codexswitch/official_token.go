...
func (s *Service) refreshOfficialProfileAuth(profile storedProfile) (storedProfile, error) {
	...
	refreshedMeta.PlanType = profile.Meta.PlanType
...
func (s *Service) syncActiveProfileToCurrentCodexHome(original, updated storedProfile) {
	...
	if err := s.writeTargetProfile(settings.CodexHomePath, s.buildSnapshotFromExistingProfile(updated), targetConfigWritePreserve); err != nil {
