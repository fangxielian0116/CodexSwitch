...
`
	...
	officialSnapshot, err := buildProfileSnapshot(
		buildTestOfficialAuthRaw(t, idToken, accessToken, "refresh-one", "account-one"),
		sharedConfigRaw,
		profileSourceImportedCurrent,
		service.now(),
	)
	...
	for _, want := range []string{
		...
		if !strings.Contains(config, want) {
			t.Fatalf("expected switched official config.toml to contain %q, got %s", want, config)
		}
	}
	for _, notWant := range []string{
		...
		if strings.Contains(config, notWant) {
			t.Fatalf("expected switched official config.toml not to contain %q, got %s", notWant, config)
		}
	}
}
